...
import { destinations, sampleTripsPlanned, formatINR, averageTripBudget } from "@/lib/data"
    ...
    <>
      ...
      <section className="border-b border-border/60 bg-card">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-8 md:grid-cols-4 md:px-6">
          <Stat icon={<Globe2 className="h-5 w-5" />} value={`${destinations.length}`} label="Destinations" />
          <Stat icon={<Users className="h-5 w-5" />} value={`${sampleTripsPlanned.toLocaleString("en-IN")}+`} label="Trips planned" />
          <Stat icon={<Wallet className="h-5 w-5" />} value={formatINR(averageTripBudget)} label="Avg. trip budget" />
